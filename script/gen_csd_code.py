...
def generate_verilog(binary_str, input_var, output_var, input_width, output_width):
    ...
    # 生成Verilog的assign语句
    verilog_code = f"assign {output_var} ="
    # 第一个项直接跟在 = 后面
    if verilog_terms:
        first_term = verilog_terms[0].lstrip("+")  # 去掉第一个项前面的加号
        verilog_code += first_term
        # 计算 = 的位置，用于对齐
        equals_pos = verilog_code.find("=")  # 找到 = 的位置并加 1，对齐到 = 后面
    # 后续的项换行并对齐
    for term in verilog_terms[1:]:
        verilog_code += f"\n{' ' * equals_pos}{term}"
    # 添加分号
    verilog_code += ";"
    return verilog_code
